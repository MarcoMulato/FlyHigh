...
public class AdmiController implements Initializable {    
    ...
    @FXML
    private void funtionEnviarCambios(ActionEvent event) throws ClassNotFoundException{
        if(piezasAdm.getText().equals("") || TextEditar.getText().equals("")){
            Alert alert = new Alert(Alert.AlertType.WARNING);
            alert.setTitle("Alerta!");
            alert.setHeaderText("No ha ingresado los cambios propuestos.");
            alert.setContentText("Ingrese cambios o aprobación de la petición.");
            ...
            if(result.get() == ButtonType.OK){
                try{
                    ...
                    Connection con = DriverManager.getConnection("jdbc:mysql://localhost/flyhigh", "root", "");
                    Statement st = con.createStatement();
                    String sql = "UPDATE peticiones SET revisionAdmi='"+piezasAdm.getText()+"', estado='Revisión Administrador' WHERE idPeticion = '"+TextEditar.getText()+"'";
                    System.out.println("SQL: " + sql);
                    ...
                    if(resultado.equals("Aprobado")){
                        ...
                        alert1.setContentText("No se necesita hacer cambios de nuevo.");
                        ...
                        alert2.setContentText("No se necesita hacer cambios de nuevo.");
